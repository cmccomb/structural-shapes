...
FIELDS = "Type EDI_Std_Nomenclature AISC_Manual_Label W A d Ht OD bf B tw tf tnom tdes Ix Iy J b t".split()
FAMILIES = {
...
def generate(source):
    ...
    for row in reader:
        ...
        properties = [number(row, key) for key in ["W", "A", "Ix", "Iy"]]
        if row["Type"] == "2L":
            if row["J"]:
        ...
        counts[family] += 1
        lines.append(f'    {identifier} => ("{label}", "{edi}", {family}, {dimensions}, [{", ".join(properties)}], {torsion});')
    if counts != COUNTS:
